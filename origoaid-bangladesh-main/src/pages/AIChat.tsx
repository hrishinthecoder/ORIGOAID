    ...
    <div className="min-h-screen bg-background flex flex-col text-foreground selection:bg-brand-emerald/30 overflow-hidden">
      ...
      <main className="flex-1 pt-28 pb-6 flex flex-col container max-w-6xl mx-auto relative z-10 overflow-hidden">
        ...
        <div className="flex-1 min-h-[650px] max-h-[850px] bg-card/30 backdrop-blur-3xl border border-border rounded-[40px] md:rounded-[56px] overflow-hidden flex flex-col shadow-2xl mx-4 md:mx-0">
          ...
          <div 
            ref={scrollRef}
            className="flex-1 overflow-y-auto p-8 md:p-14 space-y-10 md:space-y-12 custom-scrollbar text-[13px]"
          >
            {messages.map((m, i) => (
              <motion.div
          ...
          {messages.length <= 2 && (
            <motion.div 
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="px-10 md:px-14 pb-8 flex flex-wrap gap-3 md:gap-4"
            >
              {SUGGESTIONS.map((s) => (
                <button
                  key={s}
                  onClick={() => send(s)}
                  className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] px-4 md:px-6 py-3 md:py-3.5 rounded-2xl bg-muted hover:bg-brand-emerald hover:text-white transition-all border border-border text-muted-foreground hover:border-brand-emerald shadow-sm"
                >
                  {s}
                </button>
              ))}
          ...
          {/* Input Interface */}
          <div className="p-8 md:p-14 bg-muted/20 border-t border-border/50">
            <form
              onSubmit={(e) => {
                e.preventDefault();
                send();
              }}
              className="flex gap-4 md:gap-5 max-w-4xl mx-auto relative group"
            >
              <div className="absolute left-6 md:left-8 top-1/2 -translate-y-1/2 text-brand-emerald/30 group-focus-within:text-brand-emerald transition-colors pointer-events-none">
              ...
              <input
                ...
                className="flex-1 bg-background border border-border rounded-[24px] md:rounded-[32px] pl-16 md:pl-20 pr-8 md:pr-10 py-5 md:py-6 text-sm outline-none focus:border-brand-emerald/50 transition-all placeholder:text-muted-foreground/30 shadow-inner"
              />
              <Button 
                type="submit" 
                size="icon" 
                className="w-16 h-16 md:w-20 md:h-20 rounded-[24px] md:rounded-[32px] bg-brand-emerald hover:bg-brand-emerald-dark shadow-xl shadow-brand-emerald/20 shrink-0 transition-all active:scale-95" 
